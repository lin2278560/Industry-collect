...
<script>
    ...
    export default {
        ...
        data(){
            return {
                columns: [
                    ...
                    {title: '设备运行信息', align: 'center', render: (h, params) => {return h('div', [h('span', {style: {display: 'inline-block', width: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'},
                            domProps: {title: params.row.devJson}}, params.row.devJson)])}},
                    {title: '运行时间', sortable: true, align: 'center', width: 200, render: (h, params) => {let data = this.$util.time_stamp2time_date(params.row.timestamp);
                ...
                links: [
                    {name: "上报系统运行日志", to: "/logs/logUploadSys"}
                ]
            }
        },
        mounted: function() {
        ...
        methods: {
            showData() {
                this.$https.fetchPost(this.$api.log.upload.sys.url, this.page.form).then((resp) => {
