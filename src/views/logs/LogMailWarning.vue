...
<script>
    ...
    export default {
        ...
        data(){
            return {
                columns: [
                    ...
                    {title: '告警级别', width: 180,key: 'alarmType', align: 'center'},
                    {title: '告警消息', key: 'alarmMsg', align: 'center' , align: 'center', render: (h, params) => {return h('div', [h('span', {style: {display: 'inline-block', width: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'},
                            domProps: {title: params.row.alarmMsg}}, params.row.alarmMsg)])}},
                    {title: '告警时间',width: 210, sortable: true, align: 'center', render: (h, params) => {let data = this.$util.time_stamp2time_date(params.row.alarmTime);
                ...
                links: [
                    {name: "上报邮箱告警日志", to: "/logs/logMailWarning"}
                ]
            }
        },
        mounted: function() {
        ...
        methods: {
            showData() {
                this.$https.fetchPost(this.$api.log.upload.mailSendLog.url, this.page.form).then((resp) => {
