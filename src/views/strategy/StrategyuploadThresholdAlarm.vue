    ...
    export default {
        ...
        methods: {
            ...
            getDriveLoginfo(name){
                ...
                this.$https.fetchPost(this.$api.strategy.findUploadAlarmLog.url,page).then((resp) => {
                    if (resp.data.code == 200) {
                        this.tablelogShow = true
                        this.itemList= resp.data.data.data
                        console.log(resp.data.data,"data.data")
                        console.log(this.itemList.length,'itemList')
            ...
            //获取上报系统告警统计
            getData(bool){
                this.$https.fetchGet(this.$api.strategy.uploadSystemAlarmCount.url).then((resp) => {
                    if (resp.data.code == 200) {
