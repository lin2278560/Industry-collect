    ...
    <div>
        <div style="border:1px solid #666;width:100%;position:relative;float: left;margin: 10px 0;" v-for="(value,i) in links" :key="i">
            ...
            <div style="margin: 15px 10px;display:flex;">
                <div v-if="value.child.length == 0">
                    <div style="margin-left:31px;font-size:12px;">暂无数据运算</div>
                </div>
                <div v-else>
                ...
                <div v-if="value.arr">
                        <div style="border:1px solid #666;width:100%;position:relative;float: left;margin: 10px 0;">
                            <span style="background:#faf8f7; display:block; height:20px; width:100px; position:absolute; left:5px; top:-10px; text-align:center;font-size: 12px;">{{v.title}}</span>
                            <div style="margin: 15px 10px;">
                                <div style="float: left;margin: 0 0 10px 10px;" v-for="(j,i) in v.child" :key="i">
                                <span style="margin-right: 15px;font-size: 12px;">{{j.name}}&nbsp;&nbsp;:&nbsp;&nbsp;{{j.val}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div v-else>
                        <span style="margin-right: 15px;font-size: 12px;">{{v.name}}&nbsp;&nbsp;:&nbsp;&nbsp;{{v.val}}</span>
                    </div>
                </div>
                </div>
