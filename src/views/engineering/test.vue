<template>
    <div style="min-width:200px;height: 767px;float: left;border: 1px solid rgba(0,0,0,.2);box-shadow: 2px 3px 5px rgba(0,0,0,.3);border-radius: 4px;margin: 15px 0 0 20px;background-color: #ffffff;        overflow:auto;">
            ...
            delDrive() {
                if(this.item.enable) {
                    ...
                    this.$https.fetchDelete(this.$api.collect.drive.url+"/"+this.item.uuid).then((resp) => {
                        ...
                        this.$Notice.error({title: '驱动删除失败', desc: err.data.describe, duration: 2.5});
                        // this.$Notice.error({title: '请求失败', duration: 2.5});
                        console.log(err)
                    });
                }
