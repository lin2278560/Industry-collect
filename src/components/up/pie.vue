...
<script>
    ...
    export default {
        ...
        methods: {
            //拿驱动日志
            getDriveLoginfo(Duuid,Suuid,warning){
                ...
                if(this.warning != '全部'){
                    page.filters.push({
                        filed: "alarmType",
                        params: [this.warning+'告警']
                    })
                }
                this.$https.fetchPost(this.$api.strategy.findStrategyAlarmLogByPage.url,page).then((resp) => {
                    if (resp.data.code == 200) {
                        this.DriveLogData= resp.data.data
                        this.DriveLogData.title =this.driveWarningTitle
                        // this.$emit('sendDriveLogData',this.DriveLogData)
                    } else
                        this.$Notice.error({title: '数据源对象添加失败', desc: "错误代码：" + resp.data.desc, duration: 2.5});
                    }).catch(err => {
                        this.$Notice.error({title: '请求失败', desc: err, duration: 2.5});
                        console.log(err)
                    })
            },
            chart(dataArr,strId,Suuid){
                ...
                for(let i in dataArr){
                    ...
                    let obj={value:'',name:'',uuid:''}
                    obj.value = dataArr[i].riskCount + dataArr[i].warnCount
                    obj.name = dataArr[i].name   
                    obj.uuid = dataArr[i].uuid
                ...
                pie.on('click', function (params) {
                    console.log(params.name,"parmas")
                    that.driveWarningTitle = params.name
                    that.$emit('sendDriveLogData',params.data.uuid,Suuid,params.name)
                })
            },
            // eConsole(param) {
